...
# uses melan dataset
# for v3, baseline to FUP
# 
# Created: April 1 2015
# Updated: v20150421 WTL
# Used IMS: anchovy
...
proc freq data=use;
	tables
		smoke_former smoke_quit smoke_dose smoke_quit_dose
	/missing ;
run;
...
ods _all_ close;
ods htmlcss file='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1.v4.xls' style=minimal;
proc tabulate data=use missing;
	title 'tabu melanoma_c';
	class total melanoma_c
	...
	table
		...
		smoke_former coffee_c etoh_c rel_1d_cancer
		,
		(total melanoma_c)* (colpctn='Percent') /nocellmerge missing
	; 
run; 
ods htmlcss close;
ods html;
...
proc freq data=use;
	...
	tables melanoma_ins * (
		agecat race_c educ_c bmi_c physic_c  
		fmenstr menostat_c meno_age_c hyst_age_c
		parity flb_age_c oralbc_dur_c horm_nat_c  
		horm_hyst_c uvrq
		smoke_former coffee_c etoh_c rel_1d_cancer
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1ap_ins.xls' style=minimal;
...
** Continuous;
ods html body='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1b_ins.xls' style=minimal;
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1bp_ins.xls' style=minimal;
...
data Table1ap; 
	set Table1Chi Table1Trd; by Table; run;
ods html file='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1ap_mal.xls' style=minimal;
proc print data= Table1ap; 
...
** Continuous;
ods html body='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1b_mal.xls' style=minimal;
proc tabulate data=use;
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\T1\Table1bp_mal.xls' style=minimal;
proc print data= Table1T (keep=Variable Variances Probt rename=(Probt=TPvalue)
	where=(Variances='Unequal') ); 
run;
ods html close;
