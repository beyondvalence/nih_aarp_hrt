...
ods htmlcss file='C:\REB\AARP_HRTandMelanoma\Results\T1\rTable1.v3.xls' style=minimal;
proc tabulate data=use missing;
	title 'tabu melanoma_c';
	class melanoma_c total
		agecat race_c educ_c bmi_c physic_c rf_physic_c 
		fmenstr menostat_c meno_age_c hyst_age_c
		parity flb_age_c oralbc_dur_c 
		horm_nat_c horm_hyst_c uvrq
	...
	table
		...
		lacey_etcurdur lacey_etdose lacey_etfreq
		,
		(total melanoma_c)* (colpctn='Percent')
	; 
run; 
ods _all_ close;
...
proc freq data=use;
	...
	tables melanoma_mal* (
		agecat exit_agerace_c educ_c bmi_c physic_c rf_physic_c 
		fmenstr menostat_c meno_age_c hyst_age_c
		parity flb_age_c oralbc_dur_c 
		horm_nat_c horm_hyst_c uvrq
		smoke_f_c smoke_quit smoke_dose
