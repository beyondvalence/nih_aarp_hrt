...
data base_fmenstr_uvrqt (keep=Parameter ClassVal0 A_HR A_LL A_UL uvr); 
	...
	title4 '20150805WED WTL';
	set base_fmenstr_uvrq_mal; 
	where ClassVal0='<=10';
run;
ods html file='C:\REB\AARP_HRTandMelanoma\Results\baseline\master\interactions\base.fmenstr.uvrq.v2.xls' style=minimal;
proc print data= base_fmenstr_uvrqt; run;
ods _all_ close; ods html;

*******************************************************************************;
** FMENSTR Analysis by UVRQ  base  trend                 ***********************;
*******************************************************************************;
** UVRQ = 1 ****************************;
** uvrq_me * fmenstr;
...
data base_fmenstr_uvrq_mal; 
	...
	; 
run;
data base_fmenstr_uvrqt; 
	title1 'AARP Melanoma Baseline';
	title2 'P Trends for Age at Menarche <=10 vs 15+';
	title3 'By UVQR quartile';
	title4 '20150805WED WTL';
	set base_fmenstr_uvrq_mal; 
run;
ods html file='C:\REB\AARP_HRTandMelanoma\Results\baseline\master\interactions\base.fmenstr.uvrq.ptrend.v1.xls' style=minimal;
proc print data= base_fmenstr_uvrqt; run;
ods _all_ close; ods html;
...
run;
data uvrq_menopage_bins; set uvrqmenopi;
	if Effect='uvrq_me*MENOP_AGE';
	variable="uvrq_menopage_bins    ";
run;
...
proc phreg data = use multipass;
	...
	*where uvrq=1;
	ods output Type3=uvrqmenopi;
run;
data uvrq_menopage_bmal; set uvrqmenopi;
	if Effect='uvrq_me*MENOP_AGE';
	variable="uvrq_menopage_bmal     ";
run;
