...
proc freq data = use;
	tables 
		melanoma_agg melanoma_c oralbc_yn_c hormever horm_c
	/missing ;
run;
...
ods _all_ close;
ods htmlcss file='C:\REB\AARP_HRTandMelanoma\Results\T2\Table2.v1.xls' style=minimal;
proc tabulate data=use missing;
	title 'tabu melanoma_agg';
	class melanoma_c
		agecat race_c educ_c bmi_fc physic_c  
...
proc freq data=use;
	...
	tables melanoma_agg* (
		agecat race_c educ_c bmi_fc physic_c  
		fmenstr menostat_c meno_age_c hyst_age_c
		parity flb_age_c horm_c oralbc_dur_c uvrq
		smoke_f_c smoke_quit smoke_dose
		smoke_quit_dose coffee_c etoh_c rel_1d_cancer
		)	  
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\T2\Table2ap.xls' style=minimal;
...
*Continuous;
ods html body='C:\REB\AARP_HRTandMelanoma\Results\T2\Table2b.xls' style=minimal;
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\T2\Table2bp.xls' style=minimal;
