...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=parity_me/ entry = entry_age RL; 
...
proc phreg data = use multipass;
	class  parity_me (ref='1-2 live children');
	model exit_age*melanoma_mal(0)=parity_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_parity NObs = obs;
run;
...
proc phreg data = use multipass;
	class  fmenstr_me (ref='<=10');
	model exit_age*melanoma_mal(0)=fmenstr_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_fmenstr NObs = obs;
run;
...
** overall (natural + surgical menopause);
proc phreg data = use multipass;
	class  oralbc_dur_c_me (ref='never/<1yr');
...
** overall (natural + surgical menopause);
proc phreg data = use multipass;
	class  menostat_c_me (ref='natural menopause');
	model exit_age*melanoma_ins(0)=menostat_c_me/ entry = entry_age RL; 
...
proc phreg data = use multipass;
	class  menostat_c_me (ref='natural menopause');
	model exit_age*melanoma_mal(0)=menostat_c_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_menostat NObs = obs;
run;
...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=surg_age_c_me/ entry = entry_age RL; 
...
proc phreg data = use multipass;
	class  surg_age_c_me (ref='50-54');
	model exit_age*melanoma_mal(0)=surg_age_c_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_surgage NObs = obs;
run;
...
** overall (natural + surgical menopause);
proc phreg data = use multipass;
	class  horm_nat_ever_me (ref='never');
	model exit_age*melanoma_ins(0)=horm_nat_ever_me/ entry = entry_age RL; 
...
proc phreg data = use multipass;
	class  horm_nat_ever_me (ref='never');
	model exit_age*melanoma_mal(0)=horm_nat_ever_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_hormnatvr NObs = obs;
run;
...
** overall (Natural + surgical menopause);
proc phreg data = use multipass;
	class  horm_surg_ever_me (ref='never');
	model exit_age*melanoma_ins(0)=horm_surg_ever_me/ entry = entry_age RL; 
...
proc phreg data = use multipass;
	class  horm_surg_ever_me (ref='never');
	model exit_age*melanoma_mal(0)=horm_surg_ever_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_hormsurgvr NObs = obs;
run;
...
** overall (Natural + surgical menopause);
proc phreg data = use multipass;
	class  oralbc_yn_c_me (ref='never/<1yr');
	model exit_age*melanoma_ins(0)=oralbc_yn_c_me/ entry = entry_age RL; 
...
proc phreg data = use multipass;
	class  oralbc_yn_c_me (ref='never/<1yr');
	model exit_age*melanoma_mal(0)=oralbc_yn_c_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_oralbcyn NObs = obs;
run;
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\baseline\master\modelA\base_model_A.xls' style=minimal;

** put prints here;
proc print data=A_All_flb_age_c_me_ins;
	title1 underlin=1 'AARP Baseline:';
	title2 'Model A';
	title3 '20150710FRI WTL';
...
proc phreg data = use_r multipass;
	...
	model exit_age*melanoma_ins(0)=lacey_eptcurrent_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_eptcurrent_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_eptcurrent_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_eptcur NObs = obs;
run;
...
proc phreg data = use_r multipass;
	...
	model exit_age*melanoma_ins(0)=lacey_eptdur_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_eptdur_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_eptdur_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_eptdur NObs = obs;
run;
...
proc phreg data = use_r multipass;
	...
	model exit_age*melanoma_ins(0)=lacey_eptdose_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_eptdose_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_eptdose_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_eptdose NObs = obs;
run;
...
proc phreg data = use_r multipass;
	...
	model exit_age*melanoma_ins(0)=lacey_etcurrent_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_etcurrent_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_etcurrent_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_etcur NObs = obs;
run;
...
proc phreg data = use_r multipass;
	...
	model exit_age*melanoma_ins(0)=lacey_etdur_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_etdur_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_etdur_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_etdur NObs = obs;
run;
...
proc phreg data = use_r multipass;
	...
	model exit_age*melanoma_ins(0)=lacey_etdose_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_etdose_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_etdose_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_etdose NObs = obs;
run;
...
** overall (natural + surgical menopause);
proc phreg data = use_r multipass;
	class  ht_type_nat_me (ref='No HT');
	model exit_age*melanoma_ins(0)=ht_type_nat_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  ht_type_nat_me (ref='No HT');
	model exit_age*melanoma_mal(0)=ht_type_nat_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_htnat NObs = obs;
run;
...
** overall (natural + surgical menopause);
proc phreg data = use_r multipass;
	class  ht_type_surg_me (ref='No HT');
	model exit_age*melanoma_ins(0)=ht_type_surg_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  ht_type_surg_me (ref='No HT');
	model exit_age*melanoma_mal(0)=ht_type_surg_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_htsurg NObs = obs;
run;
...
** overall (Natural + surgical menopause);
proc phreg data = use_r multipass;
	class  lacey_eptcurrent_ever_me (ref='No HT');
	model exit_age*melanoma_ins(0)=lacey_eptcurrent_ever_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_eptcurrent_ever_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_eptcurrent_ever_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_eptcurvr NObs = obs;
run;
...
** overall (Natural + surgical menopause);
proc phreg data = use_r multipass;
	class  lacey_etcurrent_ever_me (ref='No HT');
	model exit_age*melanoma_ins(0)=lacey_etcurrent_ever_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  lacey_etcurrent_ever_me (ref='No HT');
	model exit_age*melanoma_mal(0)=lacey_etcurrent_ever_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_etcurvr NObs = obs;
run;
...
** overall (Natural + surgical menopause);
proc phreg data = use_r multipass;
	class  ht_type_nat_ever_me (ref='No HT');
	model exit_age*melanoma_ins(0)=ht_type_nat_ever_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  ht_type_nat_ever_me (ref='No HT');
	model exit_age*melanoma_mal(0)=ht_type_nat_ever_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_htnatvr NObs = obs;
run;
...
** overall (natural + surgical menopause);
proc phreg data = use_r multipass;
	class  ht_type_surg_ever_me (ref='No HT');
	model exit_age*melanoma_ins(0)=ht_type_surg_ever_me/ entry = entry_age RL; 
...
proc phreg data = use_r multipass;
	class  ht_type_surg_ever_me (ref='No HT');
	model exit_age*melanoma_mal(0)=ht_type_surg_ever_me/ entry = entry_age RL; 
	ods output ParameterEstimates=A_htsurgvr NObs = obs;
run;
...
ods html file='C:\REB\AARP_HRTandMelanoma\Results\rfq\master\modelA\risk_model_A.xls' style=minimal;
