...
proc phreg data = use multipass;
	...
	class eptcurrent  (ref='No HT') uvrq (ref='0 to 186.918');
	model exit_age*melanoma_ins(0)= eptcurrent
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
	ods output ParameterEstimates=crude;
run;
data crude; set crude;
	if Parameter='eptcurrent';
	variable="MainE_eptcurrent";
run;
...
proc phreg data = use multipass;
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') 
		educ_c (ref='highschool or less') bmi_c (ref='18.5 to < 25') 
		physic_c (ref='rarely');
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c physic_c
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c parity
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
...
proc phreg data = use multipass;
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') 
		educ_c (ref='highschool or less') bmi_c (ref='18.5 to < 25')
		fmenstr (ref='<=10');
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c fmenstr 
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c flb_age_c
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c oralbc_dur_c 
			/ entry = entry_age RL;
	where nat_meno_reason=1; 
...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c smoke_former 
			/ entry = entry_age RL;
	where nat_meno_reason=1; 
...
proc phreg data = use multipass;
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') 
		educ_c (ref='highschool or less') bmi_c (ref='18.5 to < 25') 
		coffee_c (ref='none');
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c coffee_c 
			/ entry = entry_age RL;
	where nat_meno_reason=1; 
...
proc phreg data = use multipass;
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') 
		educ_c (ref='highschool or less') bmi_c (ref='18.5 to < 25') etoh_c (ref='none');
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c etoh_c
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
...
proc phreg data = use multipass;
	...
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c rel_1d_cancer
			/ entry = entry_age RL; 
	where nat_meno_reason=1;
...
proc phreg data = use multipass;
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') 
		educ_c (ref='highschool or less') bmi_c (ref='18.5 to < 25') marriage (ref='married');
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c marriage
			/ entry = entry_age RL;
	where nat_meno_reason=1; 
...
data base_hormever_ins_uvr_3conf; 
	set 
		crude
		physic_c 
...
run;
data base_hormever_ins_uvr_3conf; 
	set base_hormever_ins_uvr_3conf
		(Keep= variable HazardRatio HRLowerCL HRUpperCL); 
		Parameter=log(HazardRatio);
run;
ods html file='C:\REB\AARP_HRTandMelanoma\Results\baseline\mht\base_hormever_ins_uvr_2conf.xls' style=minimal;
proc print data= base_hormever_ins_uvr_2conf; run;
ods _all_ close;ods html;
...
* baseline, melanoma in situ, exposure: horm_ever_me;
