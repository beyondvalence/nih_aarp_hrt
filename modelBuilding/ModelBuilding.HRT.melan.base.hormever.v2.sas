...
********************************************************************************;
** Analysis of 3 Potential Confounder ******************************************;
********************************************************************************;
...
********************************************************************************;
** baseline, melanoma, exp: hormever, in situ, 
** variables: uvrq + educ_c + bmi_c + marriage + smoke_former + fmenstr + 1 conf;
********************************************************************************;
** crude **;
proc phreg data = use multipass;
	title1 underlin=1 'AARP Baseline: melanoma, in situ';
	title2 'Exposure: hormever';
	*title3 'Among those with Natural Menopause Status';
	title3 'uvrq, educ_c, bmi_c + marriage + smoke_former + fmenstr + 1 confounder';
	title4 'revised parity and flb variables';
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') educ_c (ref='highschool or less')
			marriage (ref='married') bmi_c (ref='18.5 to < 25') smoke_former (ref='never smoked')
			fmenstr (ref='<=10');
	model exit_age*melanoma_ins(0)= horm_ever_me uvrq educ_c bmi_c marriage smoke_former fmenstr
			/ entry = entry_age RL;
	*where menostat_c=1;
...
proc phreg data = use multipass;
	class horm_ever_me  (ref='never') uvrq (ref='0 to 186.918') 
		...
		fmenstr (ref='<=10')
		physic_c (ref='rarely');
	model exit_age*melanoma_ins(0)=horm_ever_me uvrq educ_c bmi_c marriage smoke_former 
			fmenstr physic_c
			/ entry = entry_age RL; 
	*where menostat_c=1;
...
data base_hormever_ins_uvr_6conf; 
	set 
		crude
		physic_c 
		parity flb_age_c 
		oralbc_dur_c  coffee_c 
		etoh_c rel_1d_cancer ; 
		* uvrq education bmi_c marriage smoke_former fmenstr;
run;
data base_hormever_ins_uvr_6conf; 
	set base_hormever_ins_uvr_6conf
		(Keep= variable HazardRatio HRLowerCL HRUpperCL); 
		Parameter=log(HazardRatio);
run;
ods html file='C:\REB\AARP_HRTandMelanoma\Results\baseline\mht\base_hormever_ins_uvr_6conf1.xls' style=minimal;
proc print data= base_hormever_ins_uvr_6conf; run;
ods _all_ close;ods html;
